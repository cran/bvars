...
set.seed(1)
suppressMessages(
  specification_no2 <- specify_bvar$new(us_macro_chan)
)
run_no2             <- estimate(specification_no2, 3, 1, show_progress = FALSE)

set.seed(1)
run_no3             <- us_macro_chan |>
  specify_bvar$new() |>
...
expect_identical(
  ...
  info = "estimate_bvar: the last_draw(s) of two runs to be identical."
)

expect_identical(
  run_no1$posterior$B[1,1,1],
  run_no2$posterior$B[1,1,1],
  info = "estimate_bvar: the first draws of two runs to be identical."
)

expect_identical(
  run_no1$last_draw$starting_values$B[1,1],
  run_no3$last_draw$starting_values$B[1,1],
  info = "estimate_bvar: the last_draw(s) of a normal and pipe run to be identical."
)
